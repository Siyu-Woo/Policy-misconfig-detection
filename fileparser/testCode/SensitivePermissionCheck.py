...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="敏感权限授予低权限角色检测")
...
def main() -> None:
    ...
    found = 0
    for row in rows:
        if 80 <= row["high_pct"] < 100 and row["low_num"] > 0:
            api = row["api"]
            risk_roles = ",".join(row["low_roles"])
            print(f"Risk policy rule：{format_policy_rule(api, line_map.get(api))}")
            print(f"Risk Permission：{api} should not assigned to {risk_roles}")
            print("")
            found += 1
